...
export default function AdminHubLoader() {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center transition-opacity duration-700 bg-gradient-to-br from-[#F9A825] to-[#1F60C4] ${
        fading ? 'opacity-0' : 'opacity-100'
      }`}
    >
      <div className="relative flex items-center justify-center mb-6">
        <div className="animate-spin rounded-full h-20 w-20 border-t-4 border-b-4 border-white/40"></div>
        <span className="absolute text-4xl font-extrabold text-white tracking-widest">DGDC</span>
      </div>
